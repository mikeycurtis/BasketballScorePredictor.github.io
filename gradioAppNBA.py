...
final_modelNBA = tf.keras.models.load_model("finalModelNBA")
final_modelWNBA = tf.keras.models.load_model("finalModelWNBA")


def predictNBAGradio(df):
    home = [int(num) for num in df.iloc[0, :].values]
    home.append(1)
    home_pred = round(final_modelNBA.predict([[home]])[0][0][0])
    away = [int(num) for num in df.iloc[1, :].values]
    away.append(0)
    away_pred = round(final_modelNBA.predict([[away]])[0][0][0])
...
iface = gr.Interface(predictNBAGradio, inputs=df_input, outputs=output,
                     title="NBA Score Predictor", description=Description)
